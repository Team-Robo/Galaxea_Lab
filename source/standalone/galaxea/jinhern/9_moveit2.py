"""Pick-and-place demo with MoveIt2-planned transfer motions.

MoveIt2 (ROS 2 Jazzy) cannot be imported inside Isaac Lab's Python 3.10, so
planning runs in a separate process: start ``9_moveit2_server.py`` with the
system Python first, then run this script as usual.

    # terminal 1 (system python3, ROS 2 Jazzy environment)
    python3 source/standalone/galaxea/jinhern/9_moveit2_server.py

    # terminal 2
    ./isaaclab.sh -p source/standalone/galaxea/jinhern/9_moveit2.py

The large free-space motions (pick APPROACH, place ABOVE) are planned by
MoveIt from the measured joint state and executed waypoint by waypoint. The
short contact-critical segments (grasp descent, lift, retreat) keep the
straight-line Cartesian servoing so the end effector approaches the object
vertically. Without a reachable planning server the script falls back to the
previous pure-servoing behavior.
"""
...
parser.add_argument("--moveit_host", type=str, default="127.0.0.1")
parser.add_argument("--moveit_port", type=int, default=8765)
...
# The IK action drives left_arm_link6 shifted by this offset (see
# body_offset in ik_abs_env_cfg.py), so commands and MoveIt waypoints are
# poses of that tool point in the robot base frame.
TOOL_OFFSET = [0.0, 0.0, 0.14]


class MoveIt2Client:
    """Line-delimited JSON client for 9_moveit2_server.py."""

    def __init__(self, host, port, timeout=30.0):
        socket_connection = socket.create_connection((host, port), timeout=5.0)
        socket_connection.settimeout(timeout)
        self._file = socket_connection.makefile("rw")

    def plan(self, joint_state, position, orientation):
        """Plan for the left arm; returns EE pose waypoints or None."""
        request = {
            "type": "plan",
            "group": "left_arm",
        ...
        self._file.flush()
        response = json.loads(self._file.readline())
        if not response.get("success"):
            print(f"[MoveIt2] planning failed: {response.get('error')}")
            return None
        return response["ee_poses"]
...
class R1Manipulator:
    ...
    If a ``planner`` (MoveIt2Client) is given, the free-space transfer motions
    are executed along MoveIt-planned trajectories; otherwise every segment
    servos straight toward its target as before.
    """

    def __init__(self, env, device, num_envs, planner=None):
        self.env = env
        self.device = device
        ...
        # MoveIt trajectory for the current state: list of waypoint tensors,
        # or "FALLBACK" when planning failed and we servo directly.
    ...
    def _current_joint_state(self):
        ...
        return {
            name: float(value)
            for name, value in zip(robot.data.joint_names, positions)
        }

    def _follow_planned(self, position, orientation):
        """Move along a MoveIt trajectory toward (position, orientation).

        Plans once per state from the measured joint state (env 0), then
        commands each waypoint in turn with the usual smoothing. Falls back to
        direct servoing when no planner is available or planning fails.
        """
        position = self._as_batch(position, 3)
        orientation = self._as_batch(orientation, 4)

        if self.planner is None or self.trajectory == "FALLBACK":
            self.move_left_to(position, orientation)
            return

        if self.trajectory is None:
            waypoints = self.planner.plan(
                self._current_joint_state(),
                [float(v) for v in position[0]],
                [float(v) for v in orientation[0]],
            )
            if waypoints is None:
                print(f"[MoveIt2] falling back to direct servoing in {self.state}")
                self.trajectory = "FALLBACK"
                self.move_left_to(position, orientation)
                return
            self.trajectory = [
            ...
            self.waypoint_index = 0
            print(f"[MoveIt2] {self.state}: {len(waypoints)} waypoints")

        waypoint = self.trajectory[self.waypoint_index]
        self.move_left_to(waypoint[0:3], waypoint[3:7])
        if self.waypoint_index < len(self.trajectory) - 1 and self.left_command_reached(
            waypoint[0:3], waypoint[3:7]
        ):
    ...
    def pick(self, object_position, orientation=None):
        ...
        if self.state == "APPROACH":
            ...
            self._follow_planned(self.targets["approach"], target_orientation)
    ...
    def place(self, target_position, orientation=None):
        ...
        if self.state == "ABOVE":
            self.close_left_gripper()
            self._follow_planned(self.targets["above"], target_orientation)
...
def main():
    ...
    try:
        planner = MoveIt2Client(args_cli.moveit_host, args_cli.moveit_port)
        print(
            f"[MoveIt2] connected to planning server at "
            f"{args_cli.moveit_host}:{args_cli.moveit_port}"
        )
        if num_envs > 1:
            print("[MoveIt2] note: trajectories are planned for env 0 only")
    except OSError:
        planner = None
        print(
            "[MoveIt2] planning server not reachable, using direct servoing "
            "(start 9_moveit2_server.py with the system python3 to enable it)"
        )

    robot = R1Manipulator(env, device, num_envs, planner=planner)
